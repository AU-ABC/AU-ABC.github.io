...
![Follow these steps](./2024-06-13-instR/images/guideR.gif)
...
<img src="./2024-06-13-instR/images/" alt="R isntallation folders" width="800">
...
<img src="./2024-06-13-instR/images/rstudio.png" alt="RStudio Installer" width="600">
...
For reading an excel file a great package is the `readxl`.


:::{.callout-note}

**But wait. what are packages?** Each package consist of a set of `R` and other scripts that meet specific needs for the users of `R`. For example `readxl` reads from Excel files, which `R` cannot do on its own. There are thousands of packages out there, ranging all fields of science, and some have become very popular.

:::

Try to install the package `readxl`. You can use the command `install.packages("openxlsx")` in your RStudio console. Otherwise, go on the bottom right panel and click `Packages` and `Install` like this

<img src="./2024-06-13-instR/images/installpackage.png" alt="Installing Packages" width="1000">

Now you are ready to import an Excel file. To use the package, we can load it with `library(readxl)`. Otherwise we need to write the package name before the command to use from it (as done below). You can get a file locally on your computer or from an URL as done in this example.

```{r}
df <- openxlsx::read.xlsx("https://github.com/AU-ABC/AU-ABC.github.io/blob/681bb4eda678bc8920cbd9218bebc92e22955093/documentation/2024-06-13-instR/data/data.xlsx", sheet=1)
df
```


We can also save our files. For example we can save our gene expression data frame from before as a tab separated csv like this:

```{.r}
write.table(gene_expr,file = "data/myfirstdataframe.csv", row.names = FALSE , sep="\t")
```

and we can read it back:

```{.r}
# hint: you can use the tab of your keyboard to see a list of the available paths
df2 <- read.csv("data/myfirstdataframe.csv" , sep="\t")
df2
```
